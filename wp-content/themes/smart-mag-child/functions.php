...
class Bunyad_Theme_SmartMag
{
	...
	public function __construct() 
	{
		...
		require_once locate_template('inc/amp.php');
	...
	public function init_skins()
	{
		...
		//echo 'yoo' . Bunyad::options()->predefined_style . $style['bootstrap'];
		//exit;
	}	
	...
	public function add_post_type($args, $type = null, $atts = null)
	{
		...
		//$args['meta_key'] = 'campus';
		//$args['meta_value'] = $_SESSION['global_campus_value']; 
		// echo 'ksksksks<pre>'; print_r(['first'=>$args, 'second1'=>$type,'Third'=> $atts]); //exit;
		return $args;
	}
...
add_action( 'wp_ajax_nopriv_global_campus_select', 'post_love_add_love' );
add_action( 'wp_ajax_global_campus_select', 'post_love_add_love' );

function post_love_add_love() {
 	if(isset($_POST['val']) ){
		$_SESSION['global_campus_value'] = $_POST['val'];
	}
}
...
function wpse80087_widget_comments_args( $args)
{
	...
	if($campus_value=='null'){
		$postargs = array( 'post_type' => 'post','post_status' => 'publish');
		
	} else {
		$postargs = array( 'post_type' => 'post','post_status' => 'publish','meta_query' => array(
		       'relation' => 'AND',		         
		        array(
		            'key'     => '_select_campus',
		            'value'   => $campus_value,
	...
	$rpost=get_posts($postargs);
	$postarray=array();
	
	if(count($rpost) > 0){
		
		foreach($rpost as $rposts){
			$postarray[] = $rposts->ID;
		}
		$cposts = implode(",",$postarray);
...
function register_my_custom_menu_page() {
  ...
  add_menu_page( 'Campus', 'Campus Settings', 'manage_options', 'campus-setting.php', 'campus_setting_func',
  'dashicons-admin-generic', 200);
}

function campus_setting_func(){
	require_once locate_template('inc/campus.php');
}
