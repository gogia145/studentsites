...
if ( ! class_exists( 'FEDCP_Taxonomies' ) ) {
	/**
	 * Class FEDCP_Menu
	 */
	class FEDCP_Taxonomies {
		/**
		...
		public function fed_cp_add_custom_taxonomies() {
			...
			if ( ! isset( $request['fed_cpt_edit'] ) && isset( $merge_cpt[ $request['slug'] ] ) ) {
				wp_send_json_error( array(
					'message' => __( $merge_cpt[ $request['slug'] ] . ' Custom Post Type slug already exist' )
				) );
			}
			...
			wp_send_json_success( array(
				'message' => __( 'Taxonomy ' . $request['label'] . ' successfully ' . $status ),
				'reload'  => $redirect_url
			) );
		}
		...
		public function fed_cp_register_custom_taxonomies() {
			...
			if ( $menus ) {
				foreach ( $menus as $index => $menu ) {
					...
					$new_item_name              = fed_request_empty( $menu['new_item_name'] ) ? 'New ' . $name :
						$menu['new_item_name'];
					$popular_items              = fed_request_empty( $menu['popular_items'] ) ? 'Popular ' . $name :
						$menu['popular_items'];
				...
				<?php if ( isset( $_GET['error'] ) && $_GET['error'] === 'invalid_taxonomies' ) { ?>
					<div class="row padd_top_20">
						<div class="alert alert-danger">
							...
							<strong><?php _e( 'Sorry! You are trying to get the invalid Taxonomies, Please add new one', 'frontend-dashboard-custom-post' ); ?></strong>
						</div>
					</div>
				<?php } ?>
				<div class="row padd_top_20">
				...
				<div class="panel-heading">
					<h3 class="panel-title">
						...
						<span class="pull-right m-t-5">
											<a href="<?php echo menu_page_url( 'fed_taxonomies', false ) ?>" class="fed_add_new_custom_post">
				...
				<div class="list-group">
					...
					foreach ( $taxs as $index => $tax ) {
						$active = ( isset( $_GET['fed_type_id'] ) && $_GET['fed_type_id'] === $index ) ? 'active'
							: ''
						?>
						<a href="<?php echo menu_page_url( 'fed_taxonomies', false ) . '&fed_type_id=' . $index ?>" class="list-group-item <?php echo $active; ?>">
							<?php echo $tax['label']; ?>
						</a>
