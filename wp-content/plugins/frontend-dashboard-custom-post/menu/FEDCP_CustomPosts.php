...
if ( ! class_exists( 'FEDCP_CustomPosts' ) ) {
	/**
	 * Class FEDCP_Menu
	 */
	class FEDCP_CustomPosts {
		/**
		...
		public function fed_cp_delete_custom_post_type_delete() {
			...
			fed_verify_nonce( $_GET, 'administrator' );
			$pt = get_option( 'fed_cp_custom_posts' );
			if ( ! isset( $pt[ $request['id'] ] ) ) {
				wp_send_json_error( array( 'message' => __( 'Invalid Custom Post ID' ) ) );
			}
			$url = admin_url() . 'admin.php?page=fed_custom_post';
			unset( $pt[ $request['id'] ] );
			update_option( 'fed_cp_custom_posts', $pt );
			wp_send_json_success( array(
				'message' => __( 'Custom Post Type Successfully Deleted' ),
				'reload'  => $url
			) );
		}
		...
		public function fed_cp_add_custom_post_type_store() {
			 ...
			 * Check for Nonce
			 */
			fed_verify_nonce( $request, 'administrator' );
			/**
			 * Check for mandatory fields
			 */
			if ( ! isset( $request['slug'], $request['label'], $request['singular_name'] ) ||
			     fed_request_empty( $request['slug'] ) || fed_request_empty( $request['label'] ) || fed_request_empty( $request['singular_name'] ) ) {
				wp_send_json_error( array(
					'message' => __( 'Please fill mandatory fields' )
				) );
			}
			/**
			...
			if ( ! isset( $request['fed_cpt_edit'] ) && isset( $merge_cpt[ $request['slug'] ] ) ) {
				wp_send_json_error( array(
					'message' => __( 'Custom Post Type ' . $merge_cpt[ $request['slug'] ] . ' slug already exist' )
				) );
			}
			...
			wp_send_json_success( array(
				'message' => __( 'Custom post type ' . $request['label'] . ' successfully ' . $status ),
				'reload'  => $redirect_url
			) );
		}
		...
		public function fed_cp_register_custom_post() {
			...
			if ( $menus ) {
				foreach ( $menus as $index => $menu ) {
					...
					$archives              = fed_request_empty( $menu['archives'] ) ? $menu['singular_name'] . ' Archives' :
						$menu['archives'];
					$attributes            = fed_request_empty( $menu['attributes'] ) ? 'Attributes' :
						$menu['attributes'];
					$parent_item_colon     = fed_request_empty( $menu['parent_item_colon'] ) ? 'Parent Page:' . ' Attributes' :
						$menu['parent_item_colon'];
					$all_items             = fed_request_empty( $menu['all_items'] ) ? 'All Posts' :
						$menu['all_items'];
					$add_new_item          = fed_request_empty( $menu['add_new_item'] ) ? 'Add New ' . $name :
						$menu['add_new_item'];
					$add_new               = fed_request_empty( $menu['add_new'] ) ? 'Add New' :
						$menu['add_new'];
					$new_item              = fed_request_empty( $menu['new_item'] ) ? 'New ' . $name : $menu['new_item'];
					$edit_item             = fed_request_empty( $menu['edit_item'] ) ? 'Edit ' . $name :
						$menu['edit_item'];
					$view_item             = fed_request_empty( $menu['view_item'] ) ? 'View ' . $name :
						$menu['view_item'];
					$view_items            = fed_request_empty( $menu['view_items'] ) ? 'View ' . $menu_name :
						$menu['view_items'];
			...
			<div class="bc_fed container">
				...
				<?php if ( isset( $_GET['error'] ) && $_GET['error'] === 'invalid_post_type' ) { ?>
					<div class="row padd_top_20">
						<div class="alert alert-danger">
							<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
							<strong><?php _e( 'Sorry! You are trying to delete the invalid Custom Post Type', 'frontend-dashboard-custom-post' ); ?></strong>
						</div>
					</div>
				<?php } ?>
				<div class="row padd_top_20">
				...
				<div class="list-group">
					...
					foreach ( $pt as $index => $post ) {
						$active = ( isset( $_GET['fed_type_id'] ) && $_GET['fed_type_id'] === $index ) ? 'active'
							: ''
						?>
						<a href="<?php echo menu_page_url( 'fed_custom_post', false ) . '&fed_type_id=' . $index ?>" class="list-group-item <?php echo $active; ?>">
							<?php echo $post['label']; ?>
						</a>
